(function () {
  ...
  function GitGraph(options) {
    ...
    this.marginX = this.template.commit.dot.size * 3;
    ...
    // Compute scaling factor eagerly so it's available during commit creation
  ...
  GitGraph.prototype.render = function () {
    ...
    if (this.template.commit.message.display) {
      var _maxMsgW = 0;
      this.context.save();
      this.context.font = this.template.commit.message.font || "normal 8pt Calibri";
      for (var _k = 0; _k < this.commits.length; _k++) {
        var _w = this.context.measureText(this.commits[_k].message || "").width;
        if (_w > _maxMsgW) _maxMsgW = _w;
      }
      this.context.restore();
      unscaledResolution.x += _maxMsgW + 50;
    }
  ...
  Branch.prototype.commit = function (options) {
    ...
    if (options.detail && options.message && options.message.trim()) {
      options.detail.setAttribute("data-title", options.message.trim());
    }
    ...
    // Measure detail panel height to reserve vertical space in the graph
    if (commit.detail !== null && _isVertical(this.parent)) {
      commit.detail.style.display = "block";

      // Set left position for width calculation
      var cssMarginX = this.parent.marginX / (this.parent.scalingFactor || 1);
      var panelLeft = this.parent.canvas.offsetLeft + cssMarginX + commit.x + commit.dotSize + 20;
      commit.detail.style.left = panelLeft + "px";

      // Set explicit width matching final layout for accurate height measurement
      var section = this.parent.canvas.closest("section");
      if (section) {
        var rightPx = parseFloat(window.getComputedStyle(commit.detail).right) || 0;
        var availW = section.clientWidth - panelLeft - rightPx;
        if (availW > 50) commit.detail.style.width = availW + "px";
      }
      void commit.detail.offsetHeight; // force reflow

      var detailSpace = commit.detail.clientHeight + 40;
      commit.detail.style.width = ""; // reset so CSS right takes over

      if (this.parent.orientation === "vertical-reverse") {
        this.parent.commitOffsetY += detailSpace;
      } else {
        this.parent.commitOffsetY -= detailSpace;
      }
    }
  ...
  Branch.prototype.merge = function (target, commitOptions) {
    ...
    if (commitOptions.fastForward && isFastForwardPossible) {
      ...
      // Account for detail panel height in merge path
      var detailOffsetY = 0;
      if (commitOptions.detail) {
        var detailH = commitOptions.detail.clientHeight + 20;
        if (this.parent.orientation === "vertical") {
          detailOffsetY = -1 * detailH;
        } else if (this.parent.orientation === "vertical-reverse") {
          detailOffsetY = detailH;
        }
      }
  ...
  Commit.prototype.render = function () {
    ...
    // Detail: position panel next to its commit dot
    var DETAIL_OFFSET_TOP_IN_PX = 30;

    if (this.detail !== null && _isVertical(this.parent)) {
      var cssMarginX = this.parent.marginX / (this.parent.scalingFactor || 1);
      var cssMarginY = this.parent.marginY / (this.parent.scalingFactor || 1);
      this.detail.style.left = (this.parent.canvas.offsetLeft + cssMarginX + this.x + this.dotSize + 20) + "px";

      var detailPositionTop = (this.parent.canvas.offsetTop + cssMarginY + this.y);
      if (this.parent.orientation === "vertical-reverse") {
    ...
    if (this.messageDisplay) {
      ...
      var _msgXOff = (this.type === "mergeCommit") ? 8 : 25;
      this.context.fillText(message, this.x + this.dotSize + _msgXOff, this.y + this.dotSize / 2);
    }
  };
  ...
  function Tag(commit, options) {
    ...
    if (_isHorizontal(commit.parent)) {
      ...
      x = commit.x + commit.dotSize + 15 + textWidth / 2;
      y = commit.y;
    }
...
// A utility function to draw a rectangle with rounded corners.
  ...
  function _drawTextBG(context, x, y, text, color, font, angle, useStroke) {
    ...
    var boxH = _getFontHeight(font) + padY;

    if (useStroke) {
      context.beginPath();
      roundedRect(context, -(boxW / 2), -(boxH / 2), boxW, boxH, 4);
